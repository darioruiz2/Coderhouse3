...
def CrearBlog(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            autor = info["autor"]
            fecha = info["fecha"]
            guardar2 = Blog(titulo = titulo, subtitulo = subtitulo, texto = texto, imagen = imagen, autor = autor, fecha = fecha)
            guardar2.save()
            return render(request, "AppCoder/Inicio.html",{"mensaje444": "Blog creado con exito"})
        else:
            return render(request, "AppCoder/CrearBlog.html",{"mensaje444": "Error al crear blog"})
    else:
        form = BlogForm()   
    return render(request, "AppCoder/CrearBlog.html",{"form":form})
...
#def ObtenerImagen2(request):
#    lista = Blog.objects.filter(user = request.FILE)
#    if len(lista) !=0:
#        imagen = lista[0].imagen.url
#    else:
#         imagen = "/media/avatares/avatarpredeterminado.png"
...
def VistaBlog(request):
    ...
    blogsito = Blog.objects.all()
    
    
    return render(request, "AppCoder/Blog.html",{"blogsito": blogsito})
...
def Editblog(request,id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            blog.texto = info["texto"]
            blog.imagen = info["imagen"]
            blog.autor = info["autor"]
            blog.fecha = info["fecha"]
            blog.save()
            return render(request, "AppCoder/Inicio.html", {"mensaje555": "BLOG EDITADO CORRECTAMENTE"})
        else:
            return render(request, "AppCoder/Editblog.html", {"mensaje555": "Error al editar Blog."})
    else:
        form = BlogForm(initial={"titulo": blog.titulo, "subtitulo": blog.subtitulo, "texto": blog.texto, "imagen": blog.imagen, "autor": blog.autor, "fecha": blog.fecha})
            
    return render(request, "AppCoder/Editblog.html", {"form": form, "blog":blog})
...
def Blogdelete(request, id):
    ...
    return render(request, "AppCoder/Inicio.html",{"mensaje666": "Blog eliminado correctamente"})
